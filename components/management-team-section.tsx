...
import { managementTeam, getDepartments, type ManagementMember } from "@/data/management-team"
...
export function ManagementTeamSection() {
  ...
  const filtered =
    activeDept === "All Departments"
      ? managementTeam
      : managementTeam.filter((m) => m.department.en === activeDept)
...
function MemberCard({ member }: { member: ManagementMember }) {
  ...
  return (
    <div className="group bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="font-bold text-gray-900 text-xl mb-1 group-hover:text-[#064E3B] transition-colors">
          {t(member.name)}
        </h3>
